...
from radiopadre.render import render_title, render_url, render_preamble, render_missing
...
class TextFile(radiopadre.file.FileBase):
    ...
    def html(self,tail=None,head=None,fs=.8):
        ...
        if txt:
            return txt
        txt = render_preamble() + "<A HREF=%s target='_blank'>" % render_url(self.fullpath) + \
